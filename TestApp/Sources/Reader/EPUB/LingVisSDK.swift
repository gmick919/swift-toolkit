...
class LingVisSDK: NSObject, WKScriptMessageHandler {
  ...
  class func prepare(app: String, clientData: String) {
    LingVisSDK.app = app
  ...
  private func start() {
    ...
    webView.evaluateJavaScript("lingVisSdk.polyReadiumSignIn('', '\(LingVisSDK.token)', '', '', '\(appStr)', '\(bookIdStr)', '',  '\(clientDataStr)')")
  }
  ...
  class func signIn(email: String, password: String, newAccount: Bool, autogenerated: Bool, completion: @escaping (Result<String, Error>) -> Void) {
    ...
    let appStr = escape(str: LingVisSDK.app)
    main.webView.evaluateJavaScript("lingVisSdk.polyReadiumSignIn('\(callback)', '', '\(escape(str: email))', '\(escape(str: password))', '\(appStr)', '', \(newAccount), '', \(autogenerated))")
  }
